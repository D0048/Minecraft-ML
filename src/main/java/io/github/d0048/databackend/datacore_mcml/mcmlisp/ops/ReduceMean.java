...
public class ReduceMean extends OPBase {
    public ReduceMean() {
        setName("reduce_mean");
    ...
    public MLDataWrap runRaw(List<MLDataWrap> args) throws Exception {
        ...
        System.out.println("reduce_mean: " + Arrays.toString(data.getShape()) + " -> " + Arrays.toString(newData.getShape()));
        return newData;
    }
    ...
    static void traverseNDDataWrap(int[] previousD, MLDataWrap wrap, MLDataWrap newData, int axis2Sum) {
        ...
        if (previousD.length + 1 == wrap.getShape().length) {
            for (int i = 0; i < shape[shape.length - 1]; i++) {
                ...
                indexNew[axis2Sum] = 0;
                newData.setData(indexNew, newData.getData(indexNew) + (wrap.getData(index) / wrap.getShape()[axis2Sum]));
            }
        } else {
    ...
    public String getUsage() {
        return "Sum matrix along given axes and divide by axes length: \n    (" + getName() + "mat [indexes,to,mean]) e.g " +
                "(reduce_mean mat [1,0])";
    }
}
