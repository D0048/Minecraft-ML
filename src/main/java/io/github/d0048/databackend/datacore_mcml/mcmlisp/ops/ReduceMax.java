...
public class ReduceMax extends OPBase {
    public ReduceMax() {
        setName("reduce_max");
    ...
    public MLDataWrap runRaw(List<MLDataWrap> args) throws Exception {
        ...
        System.out.println("reduce_max: " + Arrays.toString(data.getShape()) + " -> " + Arrays.toString(newData.getShape()));
        return newData;
    }
    ...
    static void traverseNDDataWrap(int[] previousD, MLDataWrap wrap, MLDataWrap newData, int axis2Sum) {
        ...
        if (previousD.length + 1 == wrap.getShape().length) {
            for (int i = 0; i < shape[shape.length - 1]; i++) {
                ...
                indexNew[axis2Sum] = 0;
                newData.setData(indexNew, Math.max(newData.getData(indexNew), wrap.getData(index)));
            }
        } else {
    ...
    public String getUsage() {
        return "Preserve largest element in matrix along given axes: \n    (" + getName() + "mat [indexes,to,max]) e.g " +
                "(reduce_max mat [1,0])";
    }
}
