...
public class MLWand extends MLItemBase {
    ...
    public void onBlockHarvested(BlockEvent.BreakEvent e) {
        ...
        if (i != null && i.equals(mlWand)) {
            ...
            if (te instanceof MLTileEntityBase) {
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (worldIn.isRemote && entityIn instanceof EntityPlayer && currentInterval++ == effectInterval) {
            ...
            if (ss != null && !ss[0].equals(ss[1])) {
                ParticleUtil.surrondBlock(worldIn, EnumParticleTypes.REDSTONE, ss[0], 7);
                ParticleUtil.surrondBlock(worldIn, EnumParticleTypes.REDSTONE, ss[1], 7);
                // Util.fillArea(worldIn, EnumParticleTypes.VILLAGER_HAPPY, ss[0], ss[1], 1);
                ParticleUtil.surroundArea(worldIn, EnumParticleTypes.REDSTONE, ss[0], ss[1],
                        (int) (Math.sqrt(ss[0].distanceSq(ss[1])) / 2));
            }
            if (display != null) {
                ParticleUtil.surrondBlock(worldIn, EnumParticleTypes.REDSTONE, display.getPos(), 7);
            }
        }
    }
